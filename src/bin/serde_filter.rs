    ...
    F: Fn(&JsonValue) -> bool,
{
    ...
    for value in data.into_iter() {
        match value.clone() {
            JsonValue::Null => array.push(JsonValue::Null),
            JsonValue::Bool(flag) => array.push(JsonValue::Bool(flag)),
            JsonValue::Number(number) => {
                if f(&value) {
                    array.push(JsonValue::Number(number))
                }
            }
            JsonValue::String(string) => array.push(JsonValue::String(string)),
...
fn main() {
    let j = filter(
        json!([1,null,3,null,{"a": [1,2,3,4,5]}]),
        &|_|{true}
    );
        println!("{:?}", j);
}
